            ...
            if len(result) > 0:
                ...
                if poll_tuple[1] != select.POLLIN:
                    self.send_log(
                        dict(
                            level="error",
                            message=f"Unexpected poll_tuple[1]: {poll_tuple[1]}",
                        )
                    )
                else:
                    ...
                    try:
                        ...
                        self.send_log(
                            dict(
                                level="error", message=f"Failed to decode: {json_line}"
                            )
                        )
    ...
    async def _handle_received(self, recv_obj):
        ...
        if "target_id" in recv_obj:
            if recv_obj["target_id"] != self._id:
                self.send_log(
                    dict(
                        level="error", message=f"Missent object: {json.dumps(recv_obj)}"
                    )
                )
                return
        ...
        if "type" not in recv_obj or "payload" not in recv_obj:
            self.send_log(
                dict(
                    level="error",
                    message=f"Missing type or payload in object: {json.dumps(recv_obj)}",
                )
            )
            return
        ...
        if recv_obj["type"] == "user":
            ...
            self.send_log(
                dict(level="error", message=f"Unrecognised message type: {recv_obj}")
            )
    ...
    async def _handle_sys(self, sys_payload):
        if "kind" not in sys_payload:
            self.send_log(
                dict(
                    level="error",
                    message=f"Missing kind on sys_payload: {json.dumps(sys_payload)}",
                )
            )
            return
        ...
        if sys_payload["kind"] == "SYN":
            ...
            self.send_log(
                dict(
                    level="error",
                    message=f"Unknown kind on sys_payload: {json.dumps(sys_payload)}",
                )
            )
            return
